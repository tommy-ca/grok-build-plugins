...
**Bad:** `mycli deploy` then a menu that needs arrow keys.
...
- Accept stdin where it makes sense (`cat config.json | mycli config import --stdin`).
- Avoid odd positional ordering. Do not fall back to interactive prompts for missing values.
...
- Use a consistent pattern everywhere, for example `resource` plus `verb`. If `mycli service list` exists, `mycli deploy list` and `mycli config list` should follow the same shape.
