...
- [x] B.1 **Rewrite** repo-root `EXTERNAL-LOOP.md` to Symphony WORKFLOW.md shape:
...
- [x] B.2 Optional light `long-horizon-swarm/HARNESS.md` cross-cite to EXTERNAL-LOOP session path — do **not** clone arena — **P-parallel (gbp git; optional)**
- [x] B.3 Box SoT Session arms: Drove/Opus/eggbot — **already VERIFIED B1** — Metadata cite-only; do **not** remint skill bodies or personas B1 from gbp — **Metadata / box SoT**
- [x] B.4 Merge capability delta into tip `openspec/specs/herdr-agy-workflow-bind/`
- [x] B.5 `openspec validate herdr-agy-workflow-bind --type change --strict` (and `--all --strict` as needed) green after merge
- [x] B.6 If verify / marketplace scripts touched, keep them green; else leave untouched (marketplace unaffected)
- [x] B.7 **Prove bars Metadata/Static (claim done only if all hold):**
  - [x] P1 tip `EXTERNAL-LOOP.md` is WORKFLOW-shaped (frontmatter-equivalent runtime keys + body contract sections present) with isolation trinity distinct (CAO=cloud VM; Herd=herdr pane+kind=agy; Heavilifter=on-box worktree) and Drove owning tick/concurrency
  - [x] P2 tip `EXTERNAL-LOOP.md` contains herdr→agy default session path + bare-print exception + herd-with-herdr + delegate-to-agy cites
  - [x] P3 Drove B1 skills already green (cite-only); personas B1 not reminted
  - [x] P4 no thermos/lhs/gbp-external-loop-docs remint; no LIVE invent; no Elixir daemon; no skill-body dump
  - [x] P5 OpenSpec intent-driven; validate `--strict` green; tip ≥ `4c126d8e`; verify scripts green if touched
  - [x] NOT predicates: thin-only cite regress; isolation trinity collapsed; remint closed archives; invent LIVE; dual orch; propose PR claimed as apply
